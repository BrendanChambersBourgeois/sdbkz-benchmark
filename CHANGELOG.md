...
### Fixed
- **INC-55 — g6k CI SIGILL (exit 132), UNRESOLVED.** The g6k siever `import` intermittently `SIGILL`s on the v2-only GitHub runners the fleet drifted to include. Set `Dockerfile.g6k` CFLAGS/CXXFLAGS to `-march=x86-64-v2`, but disassembly of the resulting `.so` shows g6k's own build ignores those env flags and still compiles AVX2/FMA (`ymm`/`vfmadd` present), so the binary remains AVX2 and still faults on v2-only runners — the pin did not take effect (the determinism SHAs matched across "v3" and "v2" builds precisely because the binary never changed). A real fix (pin the `g6k-build-and-verify` job to guaranteed-AVX2 hardware, or force g6k's own configure to a portable ISA) is pending an owner decision. See the incident log.
