...
### Fixed
- **INC-55 — g6k CI SIGILL (exit 132).** The g6k determinism binary was built `-march=x86-64-v3`, which `SIGILL`ed on `import g6k.siever` on the v2-only GitHub runners the fleet drifted to include (3 consecutive failures; the rerun-once mitigation stopped clearing it). Dropped the `Dockerfile.g6k` build target to `-march=x86-64-v2` (universal x86-64 floor) and re-captured the determinism reference at v2 — the SHAs came out identical to the retired v3 (`basis cf22519d…`/`rprof d4faf05a…`, the n=80/β60/seed42 probe is FMA-invariant), so the anchor did not move. Old v3 values retained under `reference.previous_v3_reference`. CI `g6k-build-and-verify` green.
