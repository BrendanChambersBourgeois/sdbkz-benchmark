...
- `results/seeds/ntru_g6k_vecprobe/` probe tree (24 seeds, not in the manifest); summary reports `non_exact_q_vector` and `non_exact_outside_qspan_nonq` so untouched q-vectors are not read as outliers.
...
- `Dockerfile.fplll_patched` (fplll 5.5.0 + Kahan patch + fpylll 0.6.4); `[campaigns.ntru_n127_patched]`.
- `scripts/compare_xengine.py` and the `results/validation/` tree.
- Campaign config `backend` and `seed_tag` fields; `g6k_probe.py --tours` / `--reseed`; campaigns `ntru_g6k_pilot`, `ntru_g6k_rhf`, `ntru_xeng_n89`, `ntru_xeng_tail`.
- NTRU β=20 ball-out: 1,858 seeds across 57 cells.
- `scripts/extract_dsd_onset.py`.
- `scripts/build_g6k_manifest.py`.
- Per-seed events in `logs/pipeline.jsonl` from both runners.
...
- Shared `_bkz_core.dln_trajectory` replaces three duplicate engine loops.
...
- Steam Machine bind-mount writes (`--userns=keep-id`).
- Single-instance lock shared by `forever_runner` and `onset_driver`.
- Capped scheduler records native worker crashes.
- g6k CI SIGILL on x86-64-v2 runners (`--disable-native`).
- fplll Kahan patch sign corrected; split into code and test patches.
- `Dockerfile.fplll_patched` runs as a non-root user.
- `dln_trajectory` g6k Siever seed no longer hardcoded.
- `_config` validates `seed_tag`; `compare_xengine` guards missing rhf fields.
- `lint_seed_manifest.py` no longer flags the separate trees as orphans.
- Top-level-dir test allowlist for `paper1/` + `paper2/`.
- `CITATION.cff` concept DOI corrected.
