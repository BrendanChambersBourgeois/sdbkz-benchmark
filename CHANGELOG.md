...
### Added
- **`store_short_vectors` campaign flag + `[campaigns.ntru_g6k_vecprobe]`** (2026-09-02). Opt-in: `_bkz_core._secret_recovery` now returns the three shortest exact integer reduced-basis rows, and `run_single(store_short_vectors=True)` emits `short_vectors_{bkz,sdbkz}` next to the recovery fields; off for every historical campaign, so their per-seed JSON schema is unchanged. The probe reruns seeds 1–8 of the n=179 q=4591 β=50 cell (the cheapest cell carrying the q-independent sub-q² non-secret band) into its own tree `results/seeds/ntru_g6k_vecprobe/` so the band legs can be tested offline for dense-sublattice membership (integer combination of the secret's cyclic rotations). Queued on the Steam Machine (`config/forever_worklist_steamdeck.txt`). Tests in `test_secret_recovery.py` and `test_seed_paths.py`.
- **§8 local rerun arms** `[campaigns.q3329_kahan]` / `[campaigns.q3329_control]` (owner decision 2026-09-02: the AWS Batch rerun is dropped, the workstation queue replaces it at zero cost). Same headline cell as the canonical 100-seed set (q=3329, n=100, β=30, 1000-bit MPFR, mt70, seeds 1–100) run inside source-built fplll 5.5.0 + fpylll 0.6.4 images: `sdbkz-fplll-patched:kahan-v3` (sign-corrected Kahan patch) and `sdbkz-fplll-patched:reorder-control` (new `patches/fplll_gso_reorder_control.patch`: the same loop restructuring with the compensation held at zero, so a rate difference between the arms isolates the compensation term — the 2026-04-10 "38% → 0%" was measured with the flipped, inert term). Each arm writes to its own `results/seeds/<tag>/` tree (never the SHA-locked `q3329/`), lint-excluded from the fplll manifest until per-engine manifest wiring lands with the seeds. Supporting changes: `Dockerfile.fplll_patched` gains `--build-arg GSO_PATCH`; `scripts/q3329_verify.py` gains `--seed-tag` (output tree) and `--workers` (fork pool; per-seed numerical output identical to the in-process loop, only the wall-time fields differ — verified on a 3-seed scratch cell); `run_campaign.py` forwards both, routes `q3329_*` campaigns to that runner, and re-execs ANY campaign inside `--image` when one is given (previously g6k-only). Tests in `test_run_campaign_g6k_wrap.py` and `test_seed_paths.py`.
- JOSS submission draft: `paper.md` + `paper.bib` at repo root (software-artifact paper; findings stay on Zenodo), `CODE_OF_CONDUCT.md`. Compiles with `openjournals/inara`; `/paper.pdf` gitignored. Submission gated until 2026-10-23 (JOSS six-month public-history requirement).
- **+2,464 seeds** from the two-node β=40/β=50 campaigns, all `status=completed`: `ntru` +1,988 (β=40 fill, n=109–179, q=503–7177 — canonical manifest 11,561 → 13,549), `ntru_b2` +240 (80 each at n=167/173/179), `ntru_b2_probe` +3, `ntru_g6k` +233 (G6K manifest 2,384 → 2,617). The G6K batch carries the β=50 frontier ladder at n=181 (dim 362, integer-exact recoveries as union of the two legs, SD-BKZ / BKZ in brackets): q=2593, 3083, 3407 all 0/20; q=3733 1/20 (1/0); q=4591 10/19 (7/4; seed 2 aborted on both attempts, INC-63); q=4871 12/20 (8/5); q=5381 8/19 (6/3); q=5869 6/16 (6/1); q=6491 7/12 (7/0; eight seeds aborted, 4.00× q_fat); plus the n=179 q=4591 cell completed to 20/20 under the 86,400 s seed cap, 14/20 (11/6). Every short cell is short by g6k `gauss_sieve` assertion aborts (INC-63; seeds absent, not cap-killed) and is queued for rerun on the tolerant `sdbkz-g6k:dim384-inc63` image. The batch also carries the first β=60 seed ever completed (n=181 q=4591 seed 1 on the tolerant image: 38.5 h, both legs integer-exact, 5 tolerant swaps logged, INC-63 validation). Both manifests rebuilt; `lint-manifest` and `lint_g6k_manifest` report 0 violations.
