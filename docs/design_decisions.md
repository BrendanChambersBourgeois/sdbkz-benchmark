...
3. **Compilation target: `-march=native` is banned (it bakes the build host's exact ISA and breaks cross-machine bit-identity); the intended floor is `-march=x86-64-v2`, but see the INC-55 caveat.** `Dockerfile.g6k` sets ENV `CFLAGS`/`CXXFLAGS` to `-march=x86-64-v2`. **Caveat (INC-55, open):** g6k's own build (`setup.py` → `./configure`) sets its architecture flags independently and does **not** honour those env vars — disassembly of the shipped siever `.so` shows AVX2 + FMA (`ymm`/`vfmadd`) despite the v2 env, so the binary is effectively v3-level and `SIGILL`s on the v2-only GitHub runners the fleet now includes. (This also explains why the "v3" and "v2" reference SHAs are identical — the g6k binary never actually changed between them; the earlier "FMA-invariant probe" reasoning was incorrect.) The reference SHAs in the manifest (`basis cf22519d…` / `rprof d4faf05a…`) are the AVX2-build values. The real fix is pending an owner decision: pin `g6k-build-and-verify` to guaranteed-AVX2 hardware (no SHA change), or force g6k's own configure to a portable ISA (real sieve-speed cost + a genuine determinism re-capture). Phase 0 hashes were `-march=native` and are **not** valid references here.
