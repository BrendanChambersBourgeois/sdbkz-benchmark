...
| target | scope |
|--------|-------|
| paper 2 (NTRU + cross-engine) | `paper2/` in-progress technical report: NTRU dense-sublattice-discovery (DSD) onset and a cross-engine (fplll enumeration vs G6K sieving) study. G6K wired as a second reduction engine behind a determinism-gated seam (own manifest `results/g6k_seed_manifest.json`, `lint_g6k_manifest.py`). §7 four-discrepancy reconciliation closed; core-hours corrected to recorded-time ground truth. The three §-text edits (RHF wording / bootstrap CI / R²≈0.95) from the 2026-08-12 ledger pass were applied to `paper2/latex/sdbkz_paper2.tex` on 2026-09-02. The AWS §8 rerun is DROPPED (owner decision 2026-09-02: no further AWS spend) and replaced by a LOCAL rerun on the workstation queue: `[campaigns.q3329_kahan]` (sign-corrected patch) + `[campaigns.q3329_control]` (reorder-only, no compensation), 100 seeds each, queued behind the β=50 ladder in `config/forever_worklist.txt`. Until they land the §8 mechanism claim keeps its flipped-patch caveat. |
...
| target   | scope                                                                  |
|----------|------------------------------------------------------------------------|
| paper 2 tag | Cut a release once the §-text edits land and paper2 is submission-ready. The AWS §8 rerun is dropped (owner decision 2026-09-02, no further AWS spend); the local `q3329_kahan` / `q3329_control` arms replace it. |
