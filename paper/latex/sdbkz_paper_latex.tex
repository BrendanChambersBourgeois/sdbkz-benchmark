...
The Root Hermite Factor $\delta = (\|b_1\| / \mathrm{vol}(L)^{1/d})^{1/d}$ is a scalar metric that depends only on the length of the shortest basis vector relative to the lattice volume. Our implementation stores the uprooted Hermite Factor $\mathrm{HF} = \delta^d$ and converts only at report time (since $\mathrm{RHF} = \mathrm{HF}^{1/d}$, a monotone transformation, either metric ranks algorithms identically).\footnote{Our implementation computes the Hermite Factor (without the outer $1/d$ exponent); since $\mathrm{RHF} = \mathrm{HF}^{1/d}$ is a monotone transformation, the conclusions hold for both metrics.} It captures nothing about the interior structure of the basis; the Gram--Schmidt vectors $b^*_2$ through $b^*_d$ are invisible to it. As we demonstrate, this makes it fundamentally unable to detect the type of structural improvement SD-BKZ provides.
...
The advantage peaks at $\beta=30$ at most measured dimensions, but the relationship is dimension-dependent. At $n=110$, $\beta=40$ overtakes $\beta=30$ ($+1.158$ vs $+1.112$, both at 100 seeds) before collapsing dramatically (Table~\ref{tab:beta-n-matrix}, Figure~\ref{fig:dim-scaling}).
