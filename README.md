...
- **Verify-gated seed manifest.** `results/seed_manifest.json` indexes every seed file with SHA-256, size, mtime, verified flag, per-seed advantage. `scripts/lint_seed_manifest.py` checks three invariants on every CI run — no orphan files, no ghost entries, no hash drift. 4,741 entries (4,541 in the v1.5.0 Zenodo deposit + 200 post-deposit additions from the cliff bracket extensions); 0 violations across the v1.3.x → v1.5.x → v2.0.0 release chain.
