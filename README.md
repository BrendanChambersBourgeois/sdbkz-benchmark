...
- **Verify-gated seed manifest.** `results/seed_manifest.json` indexes every seed file with SHA-256, size, mtime, verified flag, per-seed advantage. `scripts/lint_seed_manifest.py` checks three invariants on every CI run — no orphan files, no ghost entries, no hash drift. 4,701 entries (4,432 paper-frozen + 269 post-paper additions); 0 violations across the v1.3.x → v1.5.x release chain.
...
flowchart LR
  ...
  B -->|indexed by| C[results/seed_manifest.json<br/>4701 entries · SHA-256]
